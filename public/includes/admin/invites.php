            ...
            <div class="card-body flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    <form method="POST" style="display:inline">
                        ...
                        <button type="submit" name="resend_invite" class="btn btn-secondary btn-sm admin-icon-btn" title="<?= t('resend') ?>">
                            <i class="fas fa-redo"></i>
                        </button>
                    </form>
                    <button type="button" class="btn btn-secondary btn-sm invite-copy-btn admin-icon-btn" data-link="<?= escape((defined('EMAIL_BASE_URL') ? EMAIL_BASE_URL : SITE_URL) . '/register.php?token=' . $invite['token']) ?>" title="<?= t('copy_link') ?>">
                        <i class="fas fa-copy"></i>
                    </button>
                    <form method="POST" style="display:inline">
                        <?= csrfField() ?>
                        <input type="hidden" name="invite_id" value="<?= escape($invite['id']) ?>">
                        <button type="submit" name="delete_invite" class="btn btn-danger btn-sm btn-delete admin-icon-btn" data-name="<?= escape($invite['email']) ?>">
...
<?php if (!empty($usedInvites)): ?>
    <h3 class="mb-1 mt-2"><i class="fas fa-check-circle" style="color: var(--status-success);"></i> <?= t('used_invites') ?> (<?= count($usedInvites) ?>)</h3>
    <?php foreach ($usedInvites as $invite): ?>
        <div class="card mb-1" style="opacity: 0.7;">
            <div class="card-body flex items-center justify-between">
                <div>
                    <strong><?= escape($invite['email']) ?></strong>
                    <span class="badge badge-success" style="margin-left: 0.5rem;"><?= t('registered_badge') ?></span>
                    <br><small class="text-muted"><?= t('invited_label') ?> <?= date('d M Y', strtotime($invite['created_at'])) ?></small>
                </div>
                <form method="POST" style="display:inline">
                    <?= csrfField() ?>
                    <input type="hidden" name="invite_id" value="<?= escape($invite['id']) ?>">
                    <button type="submit" name="delete_invite" class="btn btn-ghost btn-sm admin-icon-btn"><i class="fas fa-trash"></i></button>
                </form>
            </div>
        </div>
    <?php endforeach; ?>
...
<?php if (!empty($expiredInvites)): ?>
    <h3 class="mb-1 mt-2"><i class="fas fa-times-circle" style="color: var(--status-danger);"></i> <?= t('expired_invites') ?> (<?= count($expiredInvites) ?>)</h3>
    <?php foreach ($expiredInvites as $invite): ?>
        <div class="card mb-1" style="opacity: 0.5;">
            <div class="card-body flex items-center justify-between">
                <div>
                    <strong><?= escape($invite['email']) ?></strong>
                    <span class="badge badge-danger" style="margin-left: 0.5rem;"><?= t('expired_badge') ?></span>
