...
1. **Prerequisites**: `config.test.php` locally, and `ANTHROPIC_API_KEY` in `build-deploy/.env` (gitignored — never commit it). If a key isn't already saved somewhere from prior setup, generate one at console.anthropic.com → Settings → API Keys; this is the same env var the real `cron-content-topup.yml` workflow uses as a GitHub Actions secret, but that secret's value can't be read back once set, so a fresh key may be needed for local use.
